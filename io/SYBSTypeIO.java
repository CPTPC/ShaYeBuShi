...
import shayebushi.entities.units.SYBSBuildPlan;
...
public class SYBSTypeIO extends TypeIO {
    ...
    public static void writeObject(Writes write, Object object){
        if(object == null){
        ...
        }else if(object instanceof TechTree.TechNode map){
            write.b(9);
            ...
            write.b(12);
            write.i(b.pos());
        }else if(object instanceof TypeIO.BuildingBox b){
            write.b(12);
            write.i(b.pos);
            ...
            write.b(17);
            write.i(u.id);
        }else if(object instanceof TypeIO.UnitBox u){
            write.b(17);
            write.i(u.id);
    ...
    public static Object readObjectBoxed(Reads read, boolean box){
        ...
        return switch(type){
            ...
            case 10 -> read.bool();
            case 11 -> read.d();
            case 12 -> !box ? world.build(read.i()) : new TypeIO.BuildingBox(read.i());
            ...
            case 17 -> !box ? Groups.unit.getByID(read.i()) : new TypeIO.UnitBox(read.i());
            case 18 -> {
                int len = read.s();
    ...
    public static int getMaxPlanss(Queue<SYBSBuildPlan> plans){
        ...
        for(int i = 0; i < used; i++){
            SYBSBuildPlan plan = plans.get(i);
    ...
    public static void writePlansQueueNett(Writes write, Queue<SYBSBuildPlan> plans){
        if(plans == null){
            write.i(-1);
            return;
        }

        int used = getMaxPlanss(plans);
    ...
    public static Queue<SYBSBuildPlan> readPlansQueuee(Reads read){
        int used = read.i();
        if(used == -1) return null;
        var out = new Queue<SYBSBuildPlan>();
        for(int i = 0; i < used; i++){
    ...
    public static SYBSBuildPlan readPlan(Reads read){
        SYBSBuildPlan current;
        ...
        if(type == 1){ //remove
            current = new SYBSBuildPlan(Point2.x(position), Point2.y(position));
        }else{ //place
            short block = read.s();
            byte rotation = read.b();
            boolean hasConfig = read.b() == 1;
            Object config = readObject(read);
            current = new SYBSBuildPlan(Point2.x(position), Point2.y(position), rotation, content.block(block));
    ...
    public static Administration.TraceInfo readTraceInfo(Reads read){
        return new Administration.TraceInfo(readString(read), readString(read), read.b() == 1, read.b() == 1, read.i(), read.i(), readStrings(read), readStrings(read));
    }
