...
public class TurretWeapon implements Cloneable{
    ...
    public Func<TurretWeapon, TurretWeaponMount> mountType = TurretWeaponMount::new;
    ...
    public TurretWeapon(Turret t, float xOffset, float yOffset){
        name = t.name ;
        reload = t.reload ;
        if (t instanceof ItemTurret i){
            ammoTypes = i.ammoTypes ;
            ammoPerShoot = i.ammoPerShot ;
        }
        x = xOffset ;
        y = yOffset ;
        minWarmup = t.minWarmup ;
        shootCone = t.shootCone ;
        shoot = t.shoot ;
        shake = t.shake ;
        shootSound=  t.shootSound ;
        chargeSound = t.chargeSound ;
        //ejectEffect = t.coolEffect;
        useAmmo = !t.consumeAmmoOnce;
        mirror = false;
        rotate = true;
        //showStatSprite = t.squareSprite;
        controllable = false;
        aiControllable = t.playerControllable;
        alwaysShooting = t.alwaysShooting;
        autoTarget = true;
        predictTarget = t.predictTarget;
        targetInterval = t.targetInterval;
        targetSwitchInterval = t.targetInterval;
        rotateSpeed = t.rotateSpeed;
        inaccuracy = t.inaccuracy;
        recoil = t.recoil;
        recoils = t.recoils;
        recoilTime = t.recoilTime;
        recoilPow = t.recoilPow;
        cooldownTime = t.cooldownTime;
        shootX = t.shootX;
        shootY = t.shootY;
        xRand = t.xRand;
        velocityRnd = t.velocityRnd;
        shootWarmupSpeed = t.shootWarmupSpeed;
        smoothReloadSpeed = t.rotateSpeed;
        linearWarmup = t.linearWarmup;
        soundPitchMin = t.soundPitchMin;
        soundPitchMax = t.soundPitchMax;
        heatColor = t.heatColor ;
        parts = ((DrawTurret)t.drawer).parts ;
    }
    public boolean hasStats(Turret u){
        return display;
    }
    ...
    public void addStats(Turret u, Table t){
        if(inaccuracy > 0){
            t.row();
        ...
        StatValues.ammo(ammoTypes).display(t);
    }
    ...
    public void drawOutline(Turret.TurretBuild unit, TurretWeaponMount mount){
        if(!outlineRegion.found()) return;

        float
                rotation = fix ? 0 : unit.rotation - 90,
        ...
        if (fix){
            ...
            realRecoil = Mathf.pow(mount.recoil, recoilPow) * recoil;
            weaponRotation  = rotation + (rotate ? mount.rotation : baseRotation);
            wx = unit.x + x + Angles.trnsx(weaponRotation, 0, -realRecoil);
            wy = unit.y + y + Angles.trnsy(weaponRotation, 0, -realRecoil);
        }
    ...
    public void draw(Turret.TurretBuild unit, TurretWeaponMount mount){
    ...
    public void addBar(Turret t){
        Rand r = new Rand() ;
        String n = String.valueOf(r.random(400)) ;
        t.addBar(n, (Turret.TurretBuild entity) ->
                new Bar(
                        "stat.ammo",
                        Pal.ammo,
                        () -> entity.items.get(ammoType) > ammoPerShoot ? 1 : 0
                )
        );

    }
    public boolean handleAmmo(Turret.TurretBuild build){
        boolean b = false ;
        for (Item i : ammoTypes.keys()){
            if (build.items.has(i) && build.items.get(i) > ammoPerShoot){
                b = true ;
                ammoType = i ;
                bullet = ammoTypes.get(ammoType) ;
                break ;
            }
        }
        return b ;
    }
    public void update(Turret.TurretBuild unit, TurretWeaponMount mount){
        boolean can = handleAmmo(unit);
        float lastReload = mount.reload;
        mount.reload = Math.max(mount.reload - Time.delta * unit.efficiency, 0);
        mount.recoil = Mathf.approachDelta(mount.recoil, 0, unit.efficiency / recoilTime);
        if(recoils > 0){
            if(mount.recoils == null) mount.recoils = new float[recoils];
            for(int i = 0; i < recoils; i++){
                mount.recoils[i] = Mathf.approachDelta(mount.recoils[i], 0, unit.efficiency / recoilTime);
            }
        }
        ...
        if(rotate && (mount.rotate || mount.shoot) && can){
            if (fix) {
                float axisX = unit.x + x,
                        axisY = unit.y + y;

                float i = Angles.angle(axisX, axisY, mount.aimX, mount.aimY) - unit.rotation;
                float ii = unit.rotation;
                //mount.targetRotation = i + ((ii >= 45 && ii < 90) ? 135 : (ii >= 90 && ii < 180) ? -45 : (ii >= 180 && ii < 270) ) ;
                mount.targetRotation = ii - 90;
        ...
        if (fix){
            weaponRotation = (rotate ? mount.rotation : baseRotation);
            mountX = unit.x + x;
            mountY = unit.y + y;
            bulletX = mountX + Angles.trnsx(weaponRotation, this.shootX, this.shootY);
            bulletY = mountY + Angles.trnsy(weaponRotation, this.shootX, this.shootY);
            shootAngle = bulletRotation(unit, mount, bulletX, bulletY) + 90;
        }
        //find a new target
        ...
        if (fix) {
            if (alwaysShooting || unit.isShooting()) mount.shoot = true;
            if (unit.isControlled()) mount.rotate = true ;
        }
        else {
        ...
        if(continuous && mount.bullet != null){
            ...
            mount.heat = Math.max(mount.heat - Time.delta * unit.efficiency / cooldownTime, 0);
            //System.out.println(mount.heat);
            if(mount.sound != null){
        ...
        if(otherSide != -1 && alternate && mount.side == flipSprite && mount.reload <= reload / 2f && lastReload > reload / 2f){
            if (unit instanceof MultiTurret.MultiTurretBuild mt) {
                mt.mounts[otherSide].side = !mt.mounts[otherSide].side;
            }
        ...
        if(mount.shoot && //must be shooting
                ...
                (!useAmmo || unit.items.has(ammoType)) && //check ammo
                ...
                (alwaysShooting || Angles.within(rotate ? mount.rotation : fix ? 0 : unit.rotation + baseRotation, mount.targetRotation, shootCone)) //has to be within the cone
        ){
            ...
            if(useAmmo){
                unit.items.remove(ammoType, ammoPerShoot);
            }
        }
    }

    protected Teamc findTarget(Turret.TurretBuild unit, float x, float y, float range, boolean air, boolean ground){
        return Units.closestTarget(unit.team, x, y, range + Math.abs(shootY), u -> u.checkTarget(air, ground), t -> ground);
    }

    protected boolean checkTarget(Turret.TurretBuild unit, Teamc target, float x, float y, float range){
        return Units.invalidateTarget(target, unit.team, x, y, range + Math.abs(shootY));
    }

    protected float bulletRotation(Turret.TurretBuild unit, TurretWeaponMount mount, float bulletX, float bulletY){
        return rotate ? mount.rotation : Angles.angle(bulletX, bulletY, mount.aimX, mount.aimY) + (unit.rotation - unit.angleTo(mount.aimX, mount.aimY)) + baseRotation;
    }

    protected void shoot(Turret.TurretBuild unit, TurretWeaponMount mount, float shootX, float shootY, float rotation){
        //unit.apply(shootStatus, shootStatusDuration);
    ...
    protected void bullet(Turret.TurretBuild unit, TurretWeaponMount mount, float xOffset, float yOffset, float angleOffset, Mover mover){
        if(!unit.isAdded()) return;

        mount.charging = false;
        float
                xSpread = Mathf.range(xRand),
                weaponRotation = fix ? 0 : unit.rotation - 90 + (rotate ? mount.rotation : baseRotation),
                mountX = unit.x + (fix ? x : Angles.trnsx(unit.rotation - 90, x, y)),
                mountY = unit.y + (fix ? y : Angles.trnsy(unit.rotation - 90, x, y)),
                ...
                shootAngle = bulletRotation(unit, mount, bulletX, bulletY) + angleOffset + (fix ? 90 : 0),
        ...
        //Entityc shooter = unit; //Pass the missile's shooter down to its bullets
        if (fix) {
    ...
    protected void handleBullet(Turret.TurretBuild unit, TurretWeaponMount mount, Bullet bullet){
