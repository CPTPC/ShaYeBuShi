...
public class ItemTurretXianZhi extends TurretXianZhi{
    public ObjectMap<Item, BulletType> ammoTypes = new OrderedMap<>();

    public ItemTurretXianZhi(String name){
        super(name);
        hasItems = true;
    }
    ...
    public void setStats(){
        ...
        stats.add(Stat.ammo, SYBSStatValues.ammo(ammoTypes));
    }
    ...
    public void setBars(){
        ...
        addBar("ammo", (ItemTurretXianZhiBuild entity) ->
                new Bar(
                        "stat.ammo",
                        Pal.ammo,
    ...
    public void init(){
        consume(new ConsumeItemFilter(i -> ammoTypes.containsKey(i)){
            ...
            public void build(Building build, Table table){
                ...
                content.items().each(i -> filter.get(i) && i.unlockedNow(),
                        item -> image.add(new ReqImage(new Image(item.uiIcon),
                                () -> build instanceof ItemTurretXianZhiBuild it && !it.ammo.isEmpty() && ((ItemEntry)it.ammo.peek()).item == item)));
            ...
            public float efficiency(Building build){
                ...
                return build instanceof ItemTurretXianZhiBuild it && !it.ammo.isEmpty() ? 1f : 0f;
            }
    ...
    public class ItemTurretXianZhiBuild extends TurretXianZhiBuild{
