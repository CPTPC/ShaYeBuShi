...
public class MultiShuangChongZuanTou extends ShuangChongZuanTou {
...
//    public ObjectMap<ItemTurret, Item> ammos = new ObjectMap<>() ;
    ...
    public float qidongshijian = 120 ;
    public MultiShuangChongZuanTou(String name) {
        super(name);
        itemCapacity = 50 ;
        warmupSpeed = 1f ;
    }
    public MultiShuangChongZuanTou(String name, Object... turret) {
        this(name);
        configurable = true ;
        saveConfig = true ;
        hasItems = true ;
        ...
        for (Object o : turret){
            ...
            if (o instanceof ItemTurret t && t.drawer instanceof DrawTurret) {
                ItemTurret copy = new ItemTurret(ShaYeBuShi.fixedRandomString((MappableContent) o)) ;
                ShaYeBuShi.clone(t, copy) ;
                //System.out.println(copy.ammoTypes == t.ammoTypes);
                DrawTurret d = new DrawTurret();
                ShaYeBuShi.clone((DrawTurret) ((ItemTurret) o).drawer, d);
                d.basePrefix = ShaYeBuShi.randomString();
                copy.drawer = d;
                turrets.add(copy);
            }
            else if (o instanceof AbilityTurret t && t.drawer instanceof DrawAbilityTurret) {
                AbilityTurret copy = new AbilityTurret(ShaYeBuShi.fixedRandomString((MappableContent) o)) ;
                ShaYeBuShi.clone(t, copy) ;
                DrawAbilityTurret d = new DrawAbilityTurret();
                ShaYeBuShi.clone((DrawAbilityTurret) ((Turret) o).drawer, d);
                d.basePrefix = ShaYeBuShi.randomString();
                copy.drawer = d;
                turrets.add(copy);
            }
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid){
        ...
        for (Object o : turrets) {
            if (o instanceof Block b) {
                Vec2 v = (Vec2)turrets.get(turrets.indexOf(b) + 1) ;
    ...
    /*
        ...
        */
    public class MultiShuangChongZuanTouBuild extends ShuangChongZuanTouBuild implements ControlBlock {
        public BlockUnitc unit = (BlockUnitc) UnitTypes.block.create(team);
        public ObjectMap<Payload, Vec2> pay = new ObjectMap<>() ;
        public Vec2 targetPos = new Vec2() ;
        public float logicControlTime = -1;
        public boolean logicShooting = false;
        public float timer = 114514 ;
        @Override
        public void updateTile(){
            if (timer <= qidongshijian) {
                timer += Time.delta;
            }
            ...
            super.updateTile();
            //System.out.println((Mathf.lerp(1f, liquidBoostIntensity, optionalEfficiency) * efficiency) + " " + lastDrillSpeed + " " + warmup + " " + dominantItems);
            if (items.total() >= itemCapacity) {
                updateDrill();
                updateBeamDrill() ;
            }
            if (dominantItem != null) {
                items.set(dominantItem, Math.min(items.get(dominantItem), itemCapacity));
            }
            if (lastItem != null) {
                items.set(lastItem, Math.min(items.get(lastItem), itemCapacity));
            }
        }
        @Override
        ...
        public boolean shouldAmbientSound(){
            return efficiency > 0.01f && shouldConsume();
        }
        @Override
...
//            return items.get(item) < itemCapacity && (bb || (ammoTypes.get(item) != null && totalAmmo + ammoTypes.get(item).ammoMultiplier <= maxAmmo));
        ...
        public float getPowerProduction(){
            if (timer < qidongshijian
                    //&& !items.empty() && !pay.keys().toSeq().contains(p -> p instanceof BuildPayload b && b.build.items != null && b.build.items.empty()) && power.status < 1
            ) {
                //System.out.println(timer);
                return consPower.usage * Time.toSeconds ;
                //return 114514 ;
            }
        ...
        public void updateDrill(){
            ...
            if(dominantItems > 0 && efficiency > 0){
                float speed = Mathf.lerp(1f, liquidBoostIntensity, optionalEfficiency) * efficiency;

                lastDrillSpeed = (speed * dominantItems * warmup) / delay;
                warmup = Mathf.approachDelta(warmup, speed, warmupSpeed);
                lastDrillSpeed = (speed * dominantItems * speed) / delay;
                warmup = Mathf.approachDelta(speed, speed, warmupSpeed);
