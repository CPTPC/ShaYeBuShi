...
public class DanWeiZhuangPeiChang extends PayloadBlock{
    ...
    public int minTier ;
    public Seq<AssemblerUnitPlan> plans = new Seq<>(114);

    protected @Nullable ConsumePayloadDynamic consPayload;

    public DanWeiZhuangPeiChang(String name){
        super(name);
        update = solid = true;
        rotate = true;
        rotateDraw = false;
        ...
        config(Integer.class, (DanWeiZhuangPeiChangBuild tile, Integer i) -> {
            if(!configurable) return;

            if(tile.cp == i) return;
            tile.cp = i < 0 || i >= plans.size ? -1 : i;
            tile.progress = 0;
        });

        config(UnitType.class, (DanWeiZhuangPeiChangBuild tile, UnitType val) -> {
            if(!configurable) return;

            int next = plans.indexOf(p -> p.unit == val);
            if(tile.cp == next) return;
            tile.cp = next;
            tile.progress = 0;
        });
    }
    ...
    public void setBars(){
        ...
        addBar("progress", (DanWeiZhuangPeiChangBuild e) -> new Bar("bar.progress", Pal.ammo, () -> e.progress));

        addBar("units", (DanWeiZhuangPeiChangBuild e) ->
                new Bar(() ->
    ...
    public void init(){
        ...
        consume(consPayload = new ConsumePayloadDynamic((DanWeiZhuangPeiChangBuild build) -> build.plan().requirements));
    ...
    public static void assemblerUnitSpawnedd(Tile tile){
        if(tile == null || !(tile.build instanceof DanWeiZhuangPeiChangBuild build)) return;
        build.spawned();
    }

    @Remote(called = Loc.server)
    public static void assemblerDroneSpawnedd(Tile tile, int id){
        if(tile == null || !(tile.build instanceof DanWeiZhuangPeiChangBuild build)) return;
        build.droneSpawned(id);
    }

    public class DanWeiZhuangPeiChangBuild extends PayloadBlockBuild<Payload>{
