...
import shayebushi.io.SYBSTypeIO;
...
public class DieTaiUnitEntity extends UnitEntity {
    ...
    protected transient SYBSBuildPlan lastActive;
    ...
    public Queue<SYBSBuildPlan> plans = new Queue<>(1);
    public DieTaiUnitEntity() {
    }
    ...
    public void read(Reads read) {
        ...
        if(REV == 0) {
            ...
            this.mineTile = mindustry.io.TypeIO.readTile(read);
            this.mounts = mindustry.io.TypeIO.readMounts(read, this.mounts);
            this.plans = SYBSTypeIO.readPlansQueuee(read);
            this.rotation = read.f();
            this.shield = read.f();
            this.spawnedByCore = read.bool();
            this.stack = mindustry.io.TypeIO.readItems(read, this.stack);
    ...
    public boolean shouldSkip(SYBSBuildPlan plan, Building core) {
    ...
    public SYBSBuildPlan buildPlan() {
    ...
    public static DieTaiUnitEntity create() {
        return new DieTaiUnitEntity();
    }
    ...
    public void addBuild(SYBSBuildPlan place) {

        addBuild(place, true);
    }

    public void addBuild(SYBSBuildPlan place, boolean tail) {

        if (!canBuild()) return;
        SYBSBuildPlan replace = null;
        for (SYBSBuildPlan plan : plans) {
    ...
    public void drawBuildPlans() {
        ...
        Boolf<SYBSBuildPlan> skip = (plan)->plan.progress > 0.01F || (buildPlan() == plan && plan.initialized && (within(plan.x * tilesize, plan.y * tilesize, type.buildRange) || state.isEditor()));
        for (int i = 0; i < 2; i++) {
            for (SYBSBuildPlan plan : plans) {
    ...
    public void drawBuilding() {
        ...
        SYBSBuildPlan plan = active ? buildPlan() : lastActive;
        Tile tile = plan.tile();
        var core = team.core();
    ...
    public void drawPlan(SYBSBuildPlan plan, float alpha) {
    ...
    public void drawPlanTop(SYBSBuildPlan plan, float alpha) {
    ...
    public void updateBuildLogic() {
        ...
        while (buildCounter >= 1 && count++ < maxPerFrame) {
            ...
            SYBSBuildPlan current = buildPlan();
    ...
    public void validatePlans() {
        ...
        if (plans.size > 0) {
            Iterator<SYBSBuildPlan> it = plans.iterator();
            while (it.hasNext()) {
                SYBSBuildPlan plan = it.next();
                Tile tile = world.tile(plan.x, plan.y);
    ...
    public void writeSync(Writes write) {
        ...
        mindustry.io.TypeIO.writeTile(write, this.mineTile);
        mindustry.io.TypeIO.writeMounts(write, this.mounts);
        SYBSTypeIO.writePlansQueueNett(write, this.plans);
        write.f(this.rotation);
        write.f(this.shield);
        write.bool(this.spawnedByCore);
        mindustry.io.TypeIO.writeItems(write, this.stack);
