...
public class SYBSPhysicsProcess implements AsyncProcess {
    ...
    private Seq<PhysicsProcess.PhysicRef> refs = new Seq<>(false);
    ...
    public void process(){
        ...
        for(PhysicsProcess.PhysicRef ref : refs){
    ...
    public void end(){
        ...
        for(PhysicsProcess.PhysicRef ref : refs){
            ...
            //move by delta
            if (!(entity instanceof TouBuUnit || entity instanceof JieTic)) {
    ...
    public void init(){
        ...
        physics = new PhysicsProcess.PhysicsWorld(Vars.world.getQuadBounds(new Rect()));
    }
    ...
    public static class PhysicsWorld{
        ...
        private final QuadTree<PhysicsProcess.PhysicsWorld.PhysicsBody>[] trees = new QuadTree[layers];
        private final Seq<PhysicsProcess.PhysicsWorld.PhysicsBody> bodies = new Seq<>(false, 16, PhysicsProcess.PhysicsWorld.PhysicsBody.class);
        private final Seq<PhysicsProcess.PhysicsWorld.PhysicsBody> seq = new Seq<>(PhysicsProcess.PhysicsWorld.PhysicsBody.class);
        ...
        public void add(PhysicsProcess.PhysicsWorld.PhysicsBody body){
            bodies.add(body);
        }

        public void remove(PhysicsProcess.PhysicsWorld.PhysicsBody body){
            bodies.remove(body);
        }
        ...
        public void update(){
            ...
            for(int i = 0; i < bodySize; i++){
                PhysicsProcess.PhysicsWorld.PhysicsBody body = bodyItems[i];
                body.collided = false;
            ...
            for(int i = 0; i < bodySize; i++){
                ...
                for(int j = 0; j < size; j++){
                    PhysicsProcess.PhysicsWorld.PhysicsBody other = items[j];
