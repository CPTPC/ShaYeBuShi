...
public class SYBSMinimapRenderer extends MinimapRenderer {
    ...
    public SYBSMinimapRenderer(){
        ...
        Events.on(EventType.BuildTeamChangeEvent.class, event -> update(event.build.tile));
    }
    ...
    public void drawEntities(float x, float y, float w, float h, float scaling, boolean fullView){
        ...
        for(Unit unit : units){
            ...
            float scale = Scl.scl(1f) / 2f * scaling * 32f * ShaYeBuShi.minimapUnitSize * (unit.type == SYBSShenShengUnitTypes.shouge ? 2 : unit instanceof JieTic || unit instanceof TouBuUnit ? 1 : 1) ;
