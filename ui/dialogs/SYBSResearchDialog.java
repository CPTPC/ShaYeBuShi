...
public class SYBSResearchDialog extends ResearchDialog {
    public ObjectSet<SYBSResearchDialog.TechTreeNode> nodes = new ObjectSet<>();
    public SYBSResearchDialog.TechTreeNode root = new SYBSResearchDialog.TechTreeNode(TechTree.roots.first(), null);
    public SYBSResearchDialog.View view;
    public TechTree.TechNode lastNode = root.node;
    private boolean showTechSelect;

    public SYBSResearchDialog(){
        //super();
        //reset();
        cont.clear();
        titleTable.remove();
        ...
        cont.stack(titleTable, view = new SYBSResearchDialog.View(), itemDisplay = new ItemsDisplay()).grow();
        ...
        hidden(((SYBSPlanetDialog)ui.planet)::setup);
...
//        }).size(210f, 64f).name("database");
    ...
    public void switchTree(TechTree.TechNode node){
        ...
        nodes.clear();
        root = new SYBSResearchDialog.TechTreeNode(node, null);
        lastNode = node;
        view.rebuildAll();
    ...
    void treeLayout(){
        float spacing = 20f;
        SYBSResearchDialog.LayoutNode node = new SYBSResearchDialog.LayoutNode(root, null);
        SYBSResearchDialog.LayoutNode[] children = node.children;
        SYBSResearchDialog.LayoutNode[] leftHalf = Arrays.copyOfRange(node.children, 0, Mathf.ceil(node.children.length/2f));
        SYBSResearchDialog.LayoutNode[] rightHalf = Arrays.copyOfRange(node.children, Mathf.ceil(node.children.length/2f), node.children.length);
        ...
        for(SYBSResearchDialog.TechTreeNode n : nodes){
    ...
    void shift(SYBSResearchDialog.LayoutNode[] children, float amount){
        for(SYBSResearchDialog.LayoutNode node : children){
            node.y += amount;
            if(node.children != null && node.children.length > 0) shift(node.children, amount);
        }
    }

    void copyInfo(SYBSResearchDialog.LayoutNode node){
        node.node.x = node.x;
        node.node.y = node.y;
        if(node.children != null){
            for(SYBSResearchDialog.LayoutNode child : node.children){
                copyInfo(child);
            }
        }
    }

    void checkNodes(SYBSResearchDialog.TechTreeNode node){
        boolean locked = locked(node.node);
        if(!locked && (node.parent == null || node.parent.visible)) node.visible = true;
        node.visible = node.visible || Version.build == -1 || ShaYeBuShi.tiaoshi ;
        if (!TechTree.roots.contains(node.node) && node.parent == null) {
            node.visible = false ;
            nodes.remove(node) ;
        }
        node.selectable = selectable(node.node);
        for(SYBSResearchDialog.TechTreeNode l : node.children){
            l.visible = !locked && l.parent.visible;
            l.visible = l.visible || Version.build == -1 || ShaYeBuShi.tiaoshi ;
            checkNodes(l);
        }
    ...
    boolean selectable(TechTree.TechNode node){
        if (!TechTree.roots.contains(node) && node.parent == null) {
            nodes.removeAll(nodes.select(n -> n.node == node).toSeq()) ;
            return false ;
        }
    ...
    class LayoutNode extends TreeLayout.TreeNode<SYBSResearchDialog.LayoutNode> {
        final SYBSResearchDialog.TechTreeNode node;

        LayoutNode(SYBSResearchDialog.TechTreeNode node, SYBSResearchDialog.LayoutNode parent){
            this.node = node;
            this.parent = parent;
            this.width = this.height = nodeSize;
            if(node.children != null){
                children = Seq.with(node.children).map(t -> new SYBSResearchDialog.LayoutNode(t, this)).toArray(SYBSResearchDialog.LayoutNode.class);
            }
        }
    }

    public class TechTreeNode extends TreeLayout.TreeNode<SYBSResearchDialog.TechTreeNode> {
        public final TechTree.TechNode node;
        public boolean visible = true, selectable = true;

        public TechTreeNode(TechTree.TechNode node, SYBSResearchDialog.TechTreeNode parent){
            this.node = node;
            this.parent = parent;
            this.width = this.height = nodeSize;
            nodes.add(this);
            children = new SYBSResearchDialog.TechTreeNode[node.children.size];
            for(int i = 0; i < children.length; i++){
                children[i] = new SYBSResearchDialog.TechTreeNode(node.children.get(i), this);
            }
        }
    }
    ...
    public class View extends Group{
        ...
        public void rebuildAll(){
            ...
            for(SYBSResearchDialog.TechTreeNode node : nodes){
        ...
        public void drawChildren(){
            ...
            for(SYBSResearchDialog.TechTreeNode node : nodes){
                ...
                for(SYBSResearchDialog.TechTreeNode child : node.children){
