package shayebushi.maps.planet;
...
import shayebushi.SYBSWaves;
import shayebushi.ShaYeBuShi;

import static mindustry.Vars.*;

public class DeLeiKePlanetGenerator extends PlanetGenerator{
    //alternate, less direct generation (wip)
    public static boolean alt = false;

    public DeLeiKeBaseGenerator basegen = new DeLeiKeBaseGenerator();
    public float scl = 5f;
    public float waterOffset = 0.07f;
    public boolean genLakes = true;
    public Block[][] arr = {
            {Blocks.grass, Blocks.grass, Blocks.grass, Blocks.water, Blocks.water, Blocks.stone, Blocks.stone}
    };
    public Block[][] arrr =
            {
                    ...
                    {Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.basalt, Blocks.grass, Blocks.basalt, Blocks.hotrock, Blocks.basalt, Blocks.ice, Blocks.snow, Blocks.ice, Blocks.ice},
                    {Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.snow, Blocks.basalt, Blocks.basalt, Blocks.ice, Blocks.snow, Blocks.ice, Blocks.ice},
                    {Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.ice, Blocks.ice, Blocks.snow, Blocks.snow, Blocks.snow, Blocks.snow, Blocks.ice, Blocks.ice, Blocks.ice},
                    {Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.ice, Blocks.ice, Blocks.snow, Blocks.snow, Blocks.ice, Blocks.ice, Blocks.ice, Blocks.ice},
                    {Blocks.taintedWater, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.grass, Blocks.iceSnow, Blocks.snow, Blocks.ice, Blocks.ice, Blocks.ice, Blocks.ice, Blocks.ice},
    ...
    public float water = 2f / arr[0].length;
    ...
    public void generateSector(Sector sector){
        if (!sector.near().contains(s -> s.id == s.planet.startSector)) {
            sector.generateEnemyBase = !sector.hasBase();
        }
        ...
        sector.threat += (any ? 1 : 0) * noise * (poles > 0.23 ? poles * 18 : 1) ;
    }
    ...
    public Color getColor(Vec3 position){
        ...
        //return Color.valueOf("4dff7d") ;
    }
    @Override
    public int getSectorSize(Sector s){
        return (int)(super.getSectorSize(s) * 0.75f) ;
    }
    @Override
    ...
    protected void generate(){
        ...
        int enemySpawns = rand.random(1, Math.max((int)(sector.threat * 10), 1));
        ...
        if(Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z)*nmag + poles > 0.5f*addscl){
            ores.add(SYBSBlocks.likuangshi, SYBSBlocks.lvkuangshi);
        }
        if(Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z)*nmag + poles > 0.5f*addscl){
            ores.add(SYBSBlocks.youkuangshi
            //        , SYBSBlocks.leikuangshi
            );
        }
        if(Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z)*nmag + poles > 0.5f*addscl){
            ores.add(SYBSBlocks.hefeiliao
                    //        , SYBSBlocks.leikuangshi
            );
        }
        ...
        if(sector.hasEnemyBase()){
            basegen.generate(tiles, enemies.map(r -> tiles.getn(r.x, r.y)), tiles.get(spawn.x, spawn.y), state.rules.waveTeam, sector, difficulty * 5f);
        ...
        state.rules.waveSpacing = Math.max(Mathf.lerp(60 * 65 * 2, 60f * 60f * 1f, Math.max(difficulty - waveTimeDec, 0f)), 60 * 2 * 60);
        state.rules.waves = true;
        state.rules.env = sector.planet.defaultEnv;
        state.rules.enemyCoreBuildRadius = 600f;

        //spawn air only when spawn is blocked
        state.rules.spawns = SYBSWaves.generate(difficulty, new Rand(sector.id), state.rules.attackMode, state.rules.attackMode && spawner.countGroundSpawns() == 0, naval);
    }
    ...
    public void postGenerate(Tiles tiles){
        if(sector.hasEnemyBase()){
            ...
            if(spawner.countGroundSpawns() == 0){
                state.rules.spawns = SYBSWaves.generate(sector.threat, new Rand(sector.id), state.rules.attackMode, true, false);
            }
        }
    }
}
